...
import { X, FolderOpen, FolderSearch, RotateCw, Check, AlertCircle, HardDrive } from 'lucide-react'
...
export default function SettingsModal({ open, onClose }: Props) {
  ...
  async function handleChooseDir() {
    ...
    setErrorMsg('')
    try {
      const newDir = await window.electronAPI.chooseStorageDir()
      if (newDir) {
    ...
    <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4">
      ...
      >
        ...
        <div
          ...
          style={{ background: 'linear-gradient(90deg, #7c3aed, #d946ef, #7c3aed)' }}
        />

        {/* Header */}
        <div className="px-6 pt-5 pb-4 flex items-start justify-between" style={{ background: '#111118' }}>
          <div>
            <h2 className="text-white font-semibold text-base tracking-tight">Paramètres</h2>
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              <div
                ...
                style={{ background: 'linear-gradient(135deg, #7c3aed33, #d946ef22)', border: '1px solid #7c3aed55' }}
              >
                <HardDrive size={14} className="text-violet-400" />
              </div>
              <div>
                <p className="text-white text-sm font-medium leading-none">Répertoire de stockage</p>
                <p className="text-gray-600 text-xs mt-1">Musiques et stems extraits</p>
              </div>
            </div>
            ...
            {status === 'idle' && (
              <p className="text-gray-600 text-xs mt-2.5 flex items-center gap-1.5">
                <RotateCw size={10} />
                Changer le dossier redémarre le backend automatiquement.
              </p>
            )}
